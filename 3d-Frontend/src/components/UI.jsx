...
export const UI = ({ hidden, ...props }) => {
  ...
  const [chatMode, setChatMode] = useState("3d"); // '3d' or 'text'
              ...
              {chatHistory.length === 0 ? (
                <div className="flex flex-col items-center justify-center flex-1 mt-10 text-center">
                  ...
                  <p className="text-gray-500 max-w-md mb-10 text-lg">
                    I'm trained on all educational guidelines. Ask me anything about policies, procedures, or student care.
                  </p>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-2xl">
                    {[
                      { icon: "📋", title: "Attendance Policy", desc: "What is the waiver policy?" },
                      { icon: "🌟", title: "CARE Guidelines", desc: "Explain student care protocols" },
                      { icon: "🗓️", title: "Exam Procedures", desc: "What are the rules for finals?" },
                      { icon: "⚖️", title: "Code of Conduct", desc: "Student disciplinary actions" },
                    ].map((item, idx) => (
                      <button
                        key={idx}
                        onClick={() => {
                      ...
                      >
                        ...
                        <div>
                          <div className="font-semibold text-gray-800 group-hover:text-indigo-600 transition-colors">{item.title}</div>
                ...
                chatHistory.map((msg, index) => (
                  ...
                  >
                    ...
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                      msg.type === "user" ? "bg-indigo-600 text-white" : "bg-emerald-600 text-white"
                    }`}>
                      {msg.type === "user" ? (
                    ...
                    {/* Bubble */}
                    <div className={`max-w-[85%] lg:max-w-[75%] rounded-2xl px-6 py-4 shadow-sm ${
                      msg.type === "user"
                      ...
                      <div className={`text-[10px] mt-2 opacity-70 ${msg.type === 'user' ? 'text-indigo-100 text-right' : 'text-gray-400'}`}>
          ...
          <div className="bg-gradient-to-t from-white via-white to-transparent pb-6 pt-2 px-4">
             <div className="max-w-4xl mx-auto relative">
                ...
                <div className="text-center text-xs text-gray-400 mt-3">
                  AI can make mistakes. Please verify policy details with official documents.
                </div>
             </div>
          </div>
        </div>
      ...
      {chatMode === "3d" && (
        <div className="fixed inset-0 z-10 pointer-events-none flex flex-col justify-between p-6">
          ...
          <div className="pointer-events-auto self-start bg-white/80 backdrop-blur-md border border-white/20 p-4 rounded-2xl shadow-lg hover:bg-white transition-all">
            <h1 className="font-black text-xl text-gray-800 tracking-tight">Policy Assistant 🎓</h1>
            <p className="text-sm text-gray-600 font-medium">Interactive Avatar Mode</p>
          </div>
          ...
          <div className="absolute right-6 top-1/2 -translate-y-1/2 flex flex-col gap-3 pointer-events-auto">
            ...
            >
              ...
              <span className="absolute right-full mr-3 top-1/2 -translate-y-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Text Mode</span>
            </button>
